	...
	if MEM_LIMIT_LABEL in mem_resource:
		try:
			...
			if value < LOWER_LIMIT_MEGABYTES:
				...
				return False, {"error": "Memory limit is too low, less than " + LOWER_LIMIT_MEGABYTES + " MB"}
		except ValueError:
...
def set_interface_limit(interface_name, net):
	try:
		tc = subprocess.Popen(["tc","qdisc","add","dev",interface_name,"root","tbf","rate",net[NET_LIMIT_LABEL],"burst","1000kb","latency","100ms"],stderr=subprocess.PIPE)
		tc.wait()
		if tc.returncode == 0:
			return True, net
		else:
...
def set_node_net(container_name, net):
