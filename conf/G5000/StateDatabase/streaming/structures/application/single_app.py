...
def add_single_application(handler):
    if handler.database_exists("structures"):
        ...
        fixwindow_user1 = dict(
            type='structure',
            subtype='application',
            name="metagenomics_user1",
            guard=True,
            guard_policy="serverless",
            resources=dict(
                cpu=dict(max=4900, min=1400, guard=False),
            ...
            containers=["hibench1", "kafka2", "kafka3", "slave5", "slave6", "slave7", "slave8", "slave9"]
        )
        handler.add_structure(fixwindow_user1)
