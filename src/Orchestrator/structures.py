...
def retrieve_structure(structure_name):
    try:
        ...
        abort(404, "Structure does not exist")
...
def get_structure_resource(structure_name, resource):
    try:
        ...
        abort(404, "Structure or its resources do not exist")
...
def get_structure_parameter_of_resource(structure_name, resource, parameter):
    try:
        ...
        abort(404, "Resource or parameter do not exist")
...
def set_structure_parameter_of_resource(structure_name, resource, parameter):
    if not valid_resource(resource):
        abort(400, "Resource '{0}' is not valid".format(resource))

    if parameter not in ["max", "min"]:
        abort(400, "Invalid parameter state")
...
def set_structure_to_guarded_state(structure_name, state):
    if state not in [True, False]:
        abort(400, "Invalid guarded state")
...
def set_structure_multiple_resources_to_guard_state(structure_name, resources, state):
    ...
    if "resources" not in structure:
        abort(400, "Structure '{0}' has no resources to configure".format(structure_name))
    else:
        for resource in resources:
            if not valid_resource(resource):
                abort(400, "Resource '{0}' is not valid".format(resource))
            elif resource not in structure["resources"]:
                abort(400, "Resource '{0}' is missing in structure {1}".format(resource, structure_name))
...
def get_resources_to_change_guard_from_request(request):
    ...
    try:
        ...
        abort(400, "invalid content, must be a json object with resources as key")
    return resources
...
def get_structure_resource_limits(structure_name, resource):
    try:
        ...
        abort(404, "Structure or its limits do not exist")
...
def subscribe_container_to_app(structure_name, app_name):
    ...
    for application in apps:
        if cont_name in application["containers"]:
            abort(400, "Container '{0}' already subscribed in app '{1}'".format(cont_name, app_name))
...
def desubscribe_container_from_app(structure_name, app_name):
    ...
    if cont_name not in app["containers"]:
        abort(400, "Container '{0}' missing in app '{1}'".format(cont_name, app_name))
    else:
...
def desubscribe_container(structure_name):
    ...
    if 'disk' in structure["resources"]:
        ...
        if "disks" not in host["resources"]:
            abort(400, "Host does not have disks")
        else:
            ...
            if new_disk_load == None:
                abort(400, "Host does not have requested disk")
            if new_disk_load < 0:
                abort(400, "Host disk can't have negative load")
            else:
                new_disk_load -= 1
...
def subscribe_container(structure_name):
    ...
    # Check that all the needed data is present on the requested container
    container = {}
    for key in ["host"]:
    ...
    if "resources" not in req_cont:
        ...
        for key in ["max", "min", "current"]:
            if key not in req_cont["resources"]["cpu"] or key not in req_cont["resources"]["mem"]:
                abort(400, "Missing key '{0}' for cpu or mem resource".format(key))
            else:
        ...
        if 'disk' in req_cont["resources"]:
            ...
            for key in ["name", "path"]:
                if key not in disk:
                    abort(400, "Missing disk resource information")
                else:
    ...
    #     abort(400, "Name mismatch, call to endpoint to subscribe container with name '{0}', "
    #                "but content has name '{1}'".format(key, structure_name, cont_name))

    # Check if the container already exists
    try:
        cont = get_db().get_structure(structure_name)
        if cont:
            abort(400, "Container with this name already exists".format(key))
    except ValueError:
        pass
    ...
    try:
        ...
        abort(400, "Container host does not exist".format(key))
    host_containers = get_host_containers(host["host_rescaler_ip"], host["host_rescaler_port"], node_scaler_session,
                                          True)

    if cont_name not in host_containers:
        abort(400, "Container host does not report any container named '{0}'".format(cont_name))
    ...
    if "boundary" not in req_cont["resources"]["cpu"] or "boundary" not in req_cont["resources"]["mem"]:
        abort(400, "Missing 'boundary' for cpu or mem resource".format(key))
    else:
    ...
    if host["resources"]["cpu"]["free"] < needed_shares:
        abort(400, "Host does not have enough shares".format(key))
    ...
    if pending_shares > 0:
        abort(400, "Container host does not have enough free CPU shares as requested")
    ...
    if needed_memory > host_memory:
        abort(400, "Container host does not have enough free memory requested")
...
def subscribe_host(structure_name):
    ...
    if host["name"] != structure_name:
        abort(400, "Name mismatch, {0} != {1}".format(host["name"], structure_name))

    # Check if the host already exists
    try:
        host = get_db().get_structure(structure_name)
        if host:
            abort(400, "Host with this name already exists".format(key))
    except ValueError:
        pass
    ...
    try:
        ...
        abort(400, "Could not connect to this host, is it up and has its node scaler up?")
...
def subscribe_app(structure_name):
    ...
    try:
        ...
        if app:
            abort(400, "App with this name already exists".format(key))
    except ValueError:
        pass
    ...
    if "containers" in req_app:
        for c in req_app["containers"]:
            try:
                ...
                abort(400, "The container '{0}', which allegedly is a part of this app, does not exists".format(c))
    ...
    if "boundary" not in req_app["resources"]["cpu"] or "boundary" not in req_app["resources"]["mem"]:
        return abort(400, "Missing 'boundary' for cpu or mem resource".format(key))
    else:
